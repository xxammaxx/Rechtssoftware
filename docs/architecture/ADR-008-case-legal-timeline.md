...
CREATE TABLE IF NOT EXISTS case_legal_events (
    ...
    occurred_at TEXT,                   -- when the event actually happened (ISO date, nullable)
    known_at TEXT,                      -- when the user learned of it (ISO date, nullable)
    recorded_at TEXT NOT NULL,          -- when it was entered (ISO datetime UTC)
    title TEXT NOT NULL DEFAULT '',     -- user-visible short description
    ...
    review_status TEXT NOT NULL DEFAULT 'CANDIDATE', -- see Decision 5
    confidence TEXT NOT NULL DEFAULT 'LOW', -- confidence level: LOW, MEDIUM, HIGH
    ...
    amount TEXT NOT NULL DEFAULT '',    -- optional monetary amount (e.g., for PAYMENT_REQUESTED)
    legal_effect_note TEXT NOT NULL DEFAULT '', -- optional note on legal effects of this event
...
- `reference_confirmation_id` links `DEADLINE_STARTED` events to a user-confirmed
  reference event (see Decision 8).
- `occurred_at` and `known_at` are nullable to support events where one or both dates
  are unknown.
- `confidence`, `amount`, and `legal_effect_note` are optional metadata fields that
  enrich the event without being mandatory.
...
|---|---|---|
| `occurred_at` | When the legal event actually happened in the real world (e.g., the date on the Bescheid) | Nullable; when present, must not be in the future relative to `known_at` |
| `known_at` | When the user first learned about the event (e.g., date of delivery/receipt) | Nullable; when present, must be ≥ `occurred_at` |
| `recorded_at` | When the event was entered into PrivateLegalNavigator | Set by the system at insertion time; never NULL |

`occurred_at` and `known_at` are nullable to accommodate events where one or both dates
are not yet known at the time of recording. For example, a user may record that a
`DOCUMENT_ISSUED` event happened without knowing the exact issuance date, filling it
in later via a correction.
...
The system supports exactly 14 event types, modelled as a `StrEnum` in the domain layer:
...
PAYMENT_REQUESTED             — A payment/fee was requested
...
LEGAL_ACTION_FILED            — A legal action (Klage, Antrag) was filed
OTHER                         — Catch-all for event types not in the enumeration
```

**Note:** The original ADR-008 draft listed 11 event types including `HEARING_CONCLUDED`.
The actual v0.2.0 implementation removed `HEARING_CONCLUDED` and added `PAYMENT_REQUESTED`,
`LEGAL_ACTION_FILED`, and `OTHER` for a total of 14 types. `OTHER` serves as an escape
hatch for events not covered by the defined types. `HEARING_CONCLUDED` can be recorded
as a separate `DOCUMENT_ISSUED` or `OTHER` event with appropriate description.
...
| CONTRADICTS | Source contradicts target | New evidence contradicts prior testimony |
| TRIGGERS_DEADLINE | Source event starts a deadline clock | Bescheid triggers a 4-week appeal deadline |
| PARTIALLY_RESOLVES | Source partially resolves target | Teil-Abhilfe partially resolves a Widerspruch |
| OTHER | Catch-all for relations not in the enumeration | Custom relationship |
...
- The self-referential constraint is enforced in the domain layer
  (`LegalEventRelation.__post_init__`), not as a SQL `CHECK` constraint, which simplifies
  schema migration.
- `case_id` is denormalized onto `event_relations` for query performance — finding all
  relations for a case does not require joining through `case_legal_events`. The application
  layer enforces that both events belong to the same case.
...
- The v0.2.0 implementation added three relation types beyond the original seven:
  `CONTRADICTS`, `PARTIALLY_RESOLVES`, and `OTHER` — providing richer relationship modeling.
  The `review_status` column enables the same CANDIDATE → CONFIRMED lifecycle for relations.
...
- This is the identical state machine from ADR-002 (`ConfirmationStatus` enum: UNCONFIRMED,
  CONFIRMED, REJECTED, REVOKED, SUPERSEDED). The M7-A lifecycle uses `review_status` as the
  column name (vs. `lifecycle_status` in the original draft) and adds `CORRECTED` and
  `SUPERSEDED` as distinct states for naming clarity. `CORRECTED` marks the original record
  that has been replaced by a correction; `SUPERSEDED` marks a record replaced by a newer
...
- This is the direct analogue of `supersedes_confirmation_id` in `confirmed_reference_events`
  (ADR-002). The old record's `review_status` becomes `"corrected"` (or `"superseded"`) while
  the new record carries `review_status: "confirmed"`.
...
SELECT event_id, event_type, title, occurred_at, known_at, recorded_at,
       review_status, previous_event_id
FROM case_legal_events
WHERE case_id = ?
  AND review_status IN ('confirmed')
...
1. **`DEADLINE_STARTED` events** reference a `confirmation_id` from `confirmed_reference_events`:
   The `case_legal_events.reference_confirmation_id` column is a foreign key to
   `confirmed_reference_events(confirmation_id)`. It is defined in the main CREATE TABLE
   with an idempotent ALTER fallback for schema migration of existing databases, and is
   enforced by the SQLite schema (foreign_keys=ON). A `DEADLINE_STARTED`
   event is only valid if the referenced confirmation exists and is in `CONFIRMED` state.
   This enforces that no deadline can be asserted without a user-confirmed reference date.

2. **Event provenance:** Legal events can be linked to the documents that evidence them:
   The `case_legal_events.source_document_id` column references `documents(document_id)`
   with `ON DELETE SET NULL`. This is optional — not all events originate from a document
   (e.g., a manually entered hearing date).
...
CREATE TABLE IF NOT EXISTS case_legal_links (
    ...
    relevance_note TEXT NOT NULL DEFAULT '', -- why the user believes this norm applies
    status TEXT NOT NULL DEFAULT 'CANDIDATE', -- CANDIDATE, CONFIRMED, CORRECTED, REVOKED, SUPERSEDED
    created_at TEXT NOT NULL,
    confirmed_at TEXT,
    revoked_at TEXT,
    previous_link_id TEXT,              -- for corrections
    confirmed_by TEXT NOT NULL DEFAULT '' -- who confirmed the link
);
```

**Lifecycle:** CANDIDATE → CONFIRMED → CORRECTED → REVOKED → SUPERSEDED (same as legal events).

**Actual v0.2.0 `LegalLinkStatus` enum:** `CANDIDATE`, `CONFIRMED`, `REJECTED`, `CORRECTED`,
`REVOKED`, `SUPERSEDED`. Note that `SUPERSEDED` is included alongside `CORRECTED` — both
represent replacement scenarios, with `SUPERSEDED` for system-driven replacement and
`CORRECTED` for user-driven correction.
...
- The lifecycle ensures that norm assignments are auditable. If the user later determines
  ...
  preserved with `status: 'corrected'` for traceability.
...
- `legal_provision_id` is a foreign key to `legal_provisions(provision_id)`, enabling
  direct JOINs to the legal source corpus for evidence pack assembly.
- The `document_id` column (nullable FK to `documents`) allows links to be scoped to a
  specific document within a case, not just the case as a whole.
...
- **Schema deviation note:** The original ADR-008 draft used `norm_id` (without FK
  constraint), `norm_citation` (denormalized citation text), and `link_rationale`.
  The v0.2.0 implementation uses `legal_provision_id` (with FK to `legal_provisions`),
  omits `norm_citation` (derivable via JOIN), and renames `link_rationale` to
  `relevance_note`. This improves referential integrity at the cost of requiring a
  JOIN to display the citation text.
...
1. **More tables, more joins:** The design adds four new tables (`case_legal_events`,
    `event_relations`, `case_legal_links`, `legal_issues`). Timeline rendering requires
    a join across events and relations. For the single-user workload, this is negligible;
    for a hypothetical multi-user version, query optimization may be needed.

2. **Event type granularity trade-off:** The 14-event-type enumeration is opinionated.
    Some users may want more granular types (e.g., `BESCHEID_ERLASSEN` vs.
    `URTEIL_VERKUENDET`). The `OTHER` type serves as an escape hatch, and the
    enumeration can be extended, but the closed model means every addition requires
    a code change and migration. A tag-based system (`event_tags` table) could be
    added in the future without breaking the current design.

3. **Norm link table dependency (resolved):** The original ADR-008 draft noted that
   `case_legal_links.norm_id` referenced a norm table that did not yet exist. In the
   v0.2.0 implementation, this is resolved: `legal_provision_id` has a proper FK to
   `legal_provisions(provision_id)`, created together with the M7-A legal source
   foundation tables. The denormalized `norm_citation` column was removed in favor
   of a JOIN-based approach.
...
4. **No automatic timeline pruning:** The append-only model means the `case_legal_events`
    ...
    pattern (active-only filter + ORDER BY) benefits from covering indexes:
    ```sql
...
1. **Domain event types are in English:** The 14 event types use English identifiers
...
2. **Three copies of the lifecycle state machine:** The `ReviewStatus`/`LegalLinkStatus`
    pattern now exists in `confirmed_reference_events` (`is_revoke` column),
    `case_legal_events` (`review_status` column), and `case_legal_links` (`status` column).
    ...
    but the current approach avoids premature abstraction. The `LegalLinkStatus` enum
    includes `SUPERSEDED` alongside `CORRECTED` for finer-grained distinction between
    user-driven correction and system-driven replacement.
