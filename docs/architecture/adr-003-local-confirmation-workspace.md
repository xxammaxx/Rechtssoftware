...
Browser (127.0.0.1:{port})
  ...
  ├── POST /ui/.../confirm        → ReferenceEventService.confirm() → redirect
  ├── POST /ui/.../preview        → CalculationService.calculate_preview → preview.html
  ├── GET  /ui/.../history        → ReferenceEventService.get_history() → history.html
  │
  └── (JSON API unchanged)
